...
export class ViewCashClosingComponent implements OnInit{
    ...
    ngOnInit(): void {
        ...
        if (this.id){
            this.dataService.getCashClosingByIdV2(this.id)
                .pipe(first())
                .subscribe((cashCls: any) =>{
                    let cashClosing = this.dataService.findJsonValue(cashCls, 'json_result') || {};
                    if (cashClosing){
                        this.cashClosing = cashClosing;
                        if(this.cashClosing?.status?.id == 55){
                            this.activeOption = true;
                        } else if (this.cashClosing?.status?.id == 56){
                            this.deleteOption = true;
                        } else if (this.cashClosing?.status?.id == 57){
    ...
    setTableElements(cashClosing: CashClosing, activityLogs?: ActivityLog[]){
        ...
        this.totalIngresos = this.totalAmountShopResumes + this.totalDiscountShopResumes + this.totalDeliveryShopResumes + this.totalCreditPaymentsCash + this.totalCreditPaymentsDeposit + this.sobrante;
    ...
    onConfirmDialog(){
        ...
        if(this.confirmDialogId == 1){
            this.dataService.deleteCashClosing(this.cashClosing!.id!)
            .pipe(first())
            .subscribe({
                next: () => {
            ...
            this.dataService.verifyCashClosing(this.cashClosing!.id!)
            .pipe(first())
            .subscribe({
                next: () => {
    ...
    editCashClosing(){
        this.router.navigate(['/cashClosing/edit/' + this.cashClosing?.id], {
            queryParams: {
